...
resource "aws_ecs_task_definition" "task2" {
  ...
  container_definitions = jsonencode([
    {
      name   = "react3-container"
      image  = "875525659788.dkr.ecr.us-east-1.amazonaws.com/service3:latest" 
      cpu    = 256
      memory = 512
      portMappings = [
        {
...
resource "aws_ecs_service" "svc2" {
  ...
  load_balancer {
    ...
    container_name   = "react3-container"
    container_port   = "80"
  }
}
...
resource "aws_appautoscaling_target" "ecs_target2" {
  max_capacity       = 4
  min_capacity       = 1
  resource_id        = "service/clusterDev/react3-Service"
