...
resource "aws_ecs_task_definition" "task3" {
  ...
  container_definitions = jsonencode([
    {
      name   = "react4-container"
      image  = "875525659788.dkr.ecr.us-east-1.amazonaws.com/service4:latest" 
      cpu    = 256
      memory = 512
      portMappings = [
        {
...
resource "aws_ecs_service" "svc3" {
  ...
  load_balancer {
    ...
    container_name   = "react4-container"
    container_port   = "80"
  }
}
...
resource "aws_appautoscaling_target" "ecs_target3" {
  max_capacity       = 4
  min_capacity       = 1
  resource_id        = "service/clusterDev/react4-Service"
